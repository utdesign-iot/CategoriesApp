...
import com.example.torin.categories.R;
      ...
      protected FilterResults performFiltering(CharSequence constraint) {
        ...
        if(constraint.length() > 0) {
          ...
          for(int i = 0; i < mBeaconFilterList.size(); i++) {
            ...
            String device = metadata.urlMetadata.description;
            String category = "nope";
            if(device.contains("****Category:")) {
              String categorydirty = device.split("\\*\\*\\*\\*Category:")[1];
              category = categorydirty.split("\\*\\*\\*\\*")[0];
            }
            if(category.toLowerCase().contains("light") && constraint.equals("lights")){
              filters.addItem(mBeaconFilterList.getItem(i));
            }
            else if(category.toLowerCase().contains("door") && constraint.equals("doors")){
              filters.addItem(mBeaconFilterList.getItem(i));
            }
            else if(!category.toLowerCase().contains("door") &&
                    !category.toLowerCase().contains("light") && constraint.equals("misc")){
              filters.addItem(mBeaconFilterList.getItem(i));
            }
